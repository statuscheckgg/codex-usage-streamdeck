...
socket.addEventListener("message", async (event) => {
  ...
  switch (message.event) {
    case "willAppear":
      await onWillAppear(message);
      break;
    case "willDisappear":
      onWillDisappear(message);
      break;
    case "keyDown":
      await refreshAction(message.context, { force: true, feedback: true });
      break;
    case "didReceiveSettings":
      await onDidReceiveSettings(message);
      break;
    case "sendToPlugin":
      await onSendToPlugin(message);
      break;
  }
});
...
async function onWillAppear(message) {
  ...
  schedule(message.context);
  await refreshAction(message.context, { force: true });
}

function onWillDisappear(message) {
...
async function onDidReceiveSettings(message) {
  const action = actions.get(message.context);
  if (!action) {
    return;
  }
  action.settings = normalizeSettings(message.payload?.settings);
  schedule(message.context);
  await refreshAction(message.context, { force: true });
}

async function onSendToPlugin(message) {
  if (message.payload?.type === "refresh") {
    await refreshAction(message.context, { force: true, feedback: true });
  }
}
...
async function renderAction(action, payload) {
  ...
  send({ event: "setTitle", context: action.context, payload: { title: "" } });
}
